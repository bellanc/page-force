    ...
    base.class_eval do
      ...
      def self.generate_page_objects_for_sfdc_object
        @object_fields.each do |field|
          field_desc = @object_description.fields.find { |field_description| field_description.name.downcase.gsub(/__c|__r/, '').end_with? field.field_name.downcase }
          field_desc ||= @object_description.fields.find { |field_description| field_description.name.downcase.gsub(/__c|__r/, '').start_with? field.field_name.downcase }
